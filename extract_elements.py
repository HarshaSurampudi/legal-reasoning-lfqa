...
def keep_only_clean_rows(df):
    """
    Keep only rows where the context and generated text are both strings and not empty.
    """
...
def process_text_columns(df):
    ...
    # Iterate over each row of the "Generated text" column
    for text in df["GeneratedText"]:
        ...
        # Extract the "Legal Reasoning" content
    ...
    # Create new columns with the extracted content
    df["Question"] = questions
    df["Legal Reasoning"] = reasonings
    df["Answer"] = answers

    # Filter out rows where any of the new columns is None
...
val_df.to_csv('val_extracted.csv', index=False)

test_df = pd.read_csv('test.csv')
test_df = process_text_columns(test_df)
test_df.to_csv('test_extracted.csv', index=False)

train_df = pd.read_csv('train.csv')
train_df = process_text_columns(train_df)
train_df.to_csv('train_extracted.csv', index=False)
